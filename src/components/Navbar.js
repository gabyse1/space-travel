...
const Navbar = () => {
  ...
  return (
    <nav className="navbar__box">
      <NavLink className="navbar__brand nav-link" to="/"><img src={logo} alt="Logo" /></NavLink>
      <button type="button" className="navbar__menu-button">
        <span className="navbar__menu-bar" />
      </button>
      <ul className="navbar__menu-list">
        {
          links.map((link) => <li key={link.id} className="navbar-menu-item"><NavLink to={link.path}>{link.text}</NavLink></li>)
        }
      </ul>
    </nav>
  );
